...
-- Verificar si la especialidad de Medicina General existe, si no crearla
...
-- Insertar los 6 doctores de Medicina General con información completa
...
ON CONFLICT (slug) DO UPDATE SET
  ...
  telefono_hospital = EXCLUDED.telefono_hospital,
  telefonos_adicionales = EXCLUDED.telefonos_adicionales,
  mision = EXCLUDED.mision,
  areas_atencion = EXCLUDED.areas_atencion,
  formacion_detallada = EXCLUDED.formacion_detallada,
  reconocimientos = EXCLUDED.reconocimientos,
  horario_detallado = EXCLUDED.horario_detallado,
  has_detailed_profile = EXCLUDED.has_detailed_profile,
  updated_at = now();
